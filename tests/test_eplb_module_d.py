...
def test_plan_single_layer_migration_remote_plus_free_rider():
    ...
    plan, local_copies, sends, recvs = eplb._plan_single_layer_migration(
        ...
        rank=0,
        world_size=2,
        new_p2l_unique_layer=torch.tensor([0, -1, 2, 3], dtype=torch.int32),
    )
...
def test_plan_single_layer_migration_same_rank_replica_is_remap_only():
    ...
    plan, local_copies, sends, recvs = eplb._plan_single_layer_migration(
        ...
        rank=0,
        world_size=1,
        new_p2l_unique_layer=torch.tensor([0, -1], dtype=torch.int32),
    )
...
def test_plan_single_layer_migration_fills_changed_primary_before_alias():
    # The existing expert-0 weight is in slot-1, while placement chooses slot-0
    # as the new primary. The primary must be copied before slot-1 is reused.
    old_p2l = torch.tensor([1, 0], dtype=torch.int32)
    new_p2l = torch.tensor([0, 0], dtype=torch.int32)
    ...
    assert plan == [(0, 0)]
    assert [(a.src_slot, a.dst_slot) for a in local_copies] == [(1, 0)]
    assert sends == []
    assert recvs == []
