...
class MoriV2PrepareAndFinalize(mk.FusedMoEPrepareAndFinalize):
    ...
    def prepare(
        ...
        # Keep the exact valid-row count on device. This avoids a host sync and
        # prevents fused_moe from sorting the uninitialized arena tail.
...
class MoriV2ModularKernel(mk.FusedMoEModularKernel):
    ...
    all-ranks-decode batch it is capped at the static ``graph_bs * dp`` bound:
    MoRI sends one hidden row per (token, destination rank). The grid-bound aiter kernels (route-ksplit
    ...
    def _decode_recv_bound(self, topk_ids: torch.Tensor, arena_rows: int) -> int | None:
        ...
        Correctness / capture-safety:
          ...
          * Under uniform decode each of ``dp`` ranks holds ``graph_bs`` tokens,
            each routed to one or more experts; MoRI deduplicates equal
            destination ranks, so ``total_recv <= graph_bs * dp``. The bound therefore
        ...
        if not all_ranks_decode:
            return None
        bound = context.graph_bs * get_dp_group().world_size
