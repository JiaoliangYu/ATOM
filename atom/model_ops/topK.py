...
def is_rocm_aiter_fusion_shared_expert_enabled_for_quant_config(
    ...
    if dp_size > 1 and config.moe_ep_flatten_tp_across_dp:
        return False
    # MoRI derives a token's destination rank from the raw expert id, so the
    # single tail id the AITER path shares across ranks has no owner there.
    # `fuse_shared_expert` switches on the per-rank id translation instead.
    if (
        dp_size > 1
        and _has_module("mori")
        and config.enable_dp_attention
        and not config.fuse_shared_expert
    ):
        return False
